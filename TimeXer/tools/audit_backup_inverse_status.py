...
def _scan_dirs(backup_root: Path) -> Tuple[List[Path], List[Path]]:
    pred_true_dirs: Dict[str, Path] = {}
    metrics_only_dirs: Dict[str, Path] = {}

    for pred_path in backup_root.rglob("pred.npy"):
        d = pred_path.parent
        if (d / "true.npy").exists():
            pred_true_dirs[str(d.resolve())] = d

    for metrics_path in backup_root.rglob("metrics.csv"):
        d = metrics_path.parent
        if str(d.resolve()) in pred_true_dirs:
            continue
        if (d / "pred.npy").exists() and (d / "true.npy").exists():
            continue
        metrics_only_dirs[str(d.resolve())] = d

    return list(pred_true_dirs.values()), list(metrics_only_dirs.values())


def audit_one_pred_true_dir(d: Path) -> Dict[str, object]:
    row: Dict[str, object] = {
        "dir": str(d),
        "has_pred_true": 1,
        "has_metrics_csv": 1 if (d / "metrics.csv").exists() else 0,
        "has_metrics_npy": 1 if (d / "metrics.npy").exists() else 0,
    }

    log_path = _find_log_near(d)
    row["log_path"] = str(log_path) if log_path is not None else ""
    row["has_log_near"] = 1 if log_path is not None else 0

    exp_args = None
    if log_path is not None:
        exp_args = parse_exp_args_from_log(log_path, fallback_model_id=d.name)
    if exp_args is None:
        exp_args = parse_exp_args_from_model_id_only(d.name)
        if exp_args is None:
            row["parse_status"] = "cannot_infer_args"
            return row
        row["parse_status"] = "model_id_only"
    else:
        row["parse_status"] = "ok"

    row.update({f"arg_{k}": v for k, v in asdict(exp_args).items()})

    try:
        preds = np.load(d / "pred.npy", allow_pickle=False)
        trues = np.load(d / "true.npy", allow_pickle=False)
    except Exception as e:
        row["parse_status"] = "npy_load_error"
        row["npy_error"] = f"{type(e).__name__}:{e}"
        return row

    row["pred_shape"] = str(tuple(preds.shape))
    row["true_shape"] = str(tuple(trues.shape))
    row["pred_p99_abs"] = float(_p99_abs(preds))
    row["true_p99_abs"] = float(_p99_abs(trues))

    if exp_args.data != "custom":
        row["scale_state"] = "unsupported_data"
        return row
    ...
    try:
        ...
        if hasattr(dataset, "target_indices"):
            ...
            if target_indices:
                ...
                if trues.shape[-1] != len(target_indices) and trues.shape[-1] > max_idx:
                    trues = trues[:, :, target_indices]
                    sliced = 1
        state, info = _infer_scale_state_for_custom(dataset, exp_args, trues)
        row["scale_state"] = state
        row["sliced_to_targets"] = sliced
        row.update({f"scale_{k}": v for k, v in info.items()})
    except Exception as e:
        row["scale_state"] = "error"
        row["scale_error"] = f"{type(e).__name__}:{e}"

    return row


def audit_one_metrics_only_dir(d: Path) -> Dict[str, object]:
    return {
        "dir": str(d),
        "has_pred_true": 0,
        "has_metrics_csv": 1 if (d / "metrics.csv").exists() else 0,
        "has_metrics_npy": 1 if (d / "metrics.npy").exists() else 0,
        "parse_status": "no_pred_true",
...
def main() -> None:
    ...
    pred_true_dirs, metrics_only_dirs = _scan_dirs(backup_root)

    cpu = os.cpu_count() or 8
    workers = int(args.workers or 0) or max(4, min(16, cpu))

    rows: List[Dict[str, object]] = []
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futures = []
        for d in pred_true_dirs:
            futures.append(ex.submit(audit_one_pred_true_dir, d))
        for d in metrics_only_dirs:
            futures.append(ex.submit(audit_one_metrics_only_dir, d))

        for fut in as_completed(futures):
            rows.append(fut.result())

    rows.sort(key=lambda r: (str(r.get("scale_state", "")), str(r.get("dir", ""))))
    report_path = backup_root / "_inverse_audit_report.csv"
    ...
    counts: Dict[str, int] = {}
    for r in rows:
        k = str(r.get("scale_state", ""))
        counts[k] = counts.get(k, 0) + 1

    print(
        f"Done. pred_true_dirs={len(pred_true_dirs)} metrics_only_dirs={len(metrics_only_dirs)} "
        f"report={report_path} counts={counts}"
    )
